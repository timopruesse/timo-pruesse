<script lang="ts">
	...
	function getColorClasses(frequency: Frequency): string {
		if (frequency === Frequency.Frequently) {
			return 'bg-indigo-600 text-white hover:bg-indigo-900';
		}

		if (frequency === Frequency.LessFrequently) {
			return 'bg-yellow-400 text-yellow-900 hover:bg-yellow-600';
		}

		return 'bg-green-700 text-white hover:bg-green-900';
	}
...
<button
	...
	} hover:text-white overflow-hidden w-full sm:w-auto px-2 py-1 sm:px-6 sm:py-2 sm:text-lg rounded-full hover:z-50 transition duration-400 ease-in-out hover:scale-105 sm:hover:-skew-y-1`}
>
	<slot />
</button>
